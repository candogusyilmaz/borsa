...
public class CashActivityCommandService {
    ...
    public ActivityResponse recordCashActivity(UUID ownerUserAccountId, UUID accountId, CashActivityRequest request) {
        ...
        var hash = cashActivityFingerprint(accountId, request.activityType(), amount, request.recordingMode(), request.effectiveAt(),
                request.confirmPolicyBreach(), request.expectedBalanceVersion());
        commandLockRepository.lock(ownerUserAccountId, LedgerCommandScopes.CASH_ACTIVITY, request.clientRequestId());
        var replay = idempotencyStore.replay(request.clientRequestId(), ownerUserAccountId, LedgerCommandScopes.CASH_ACTIVITY, hash, ActivityResponse.class);
        if (replay != null) {
            return replay;
        }
    ...
    public ActivityResponse reverse(UUID ownerUserAccountId, UUID activityId, ReversalRequest request) {
        var observedAt = clock.instant();
        var hash = fingerprint.hash(fingerprint.values("activityId", activityId.toString(), "correctionReason", request.correctionReason().trim()));
        commandLockRepository.lock(ownerUserAccountId, LedgerCommandScopes.ACTIVITY_REVERSAL, request.clientRequestId());
        var replay = idempotencyStore.replay(request.clientRequestId(), ownerUserAccountId, LedgerCommandScopes.ACTIVITY_REVERSAL, hash,
                ActivityResponse.class);
        if (replay != null) {
            return replay;
        }

        var original = activityRepository.findOwnedForUpdate(activityId, ownerUserAccountId)
                .orElseThrow(() -> new AppException(LedgerErrorCode.ACTIVITY_NOT_FOUND));
        if (original.getActivityType() == ActivityType.OPENING_BALANCE || original.getActivityType() == ActivityType.REVERSAL ||
        ...
        var accountIds = originalPostings.stream().map(MoneyPosting::getFinancialAccountId).distinct().sorted().toList();
        ...
        var reversal = writeReversal(ownerUserAccountId, activityId, original, originalPostings, projections, request.correctionReason().trim(),
                request.clientRequestId(), observedAt);
        return saveResult(ownerUserAccountId, reversal, hash, request.clientRequestId(), observedAt, LedgerCommandScopes.ACTIVITY_REVERSAL);
    }

    private String cashActivityFingerprint(UUID accountId, ActivityType activityType, FinancialAmount amount, RecordingMode recordingMode, Instant effectiveAt,
            boolean confirmPolicyBreach, Long expectedBalanceVersion) {
        return fingerprint.hash(fingerprint.values("accountId", accountId.toString(), "activityType", activityType.name(), "amount", amount.canonical(),
                "recordingMode", recordingMode.name(), "effectiveAt", effectiveAt.toString(), "confirmPolicyBreach", confirmPolicyBreach,
    ...
    private ActivityResponse saveResult(UUID ownerUserAccountId, Activity activity, String hash, UUID clientRequestId, Instant observedAt, String scope) {
        entityManager.flush();
        var response = readRepository.findActivity(ownerUserAccountId, activity.getId())
                .orElseThrow(() -> new AppException(LedgerErrorCode.ACTIVITY_NOT_FOUND));
        idempotencyStore.save(ownerUserAccountId, scope, clientRequestId, hash, "ACTIVITY", activity.getId(), response, observedAt);
        return response;
    }
    ...
    private static void requireCashActionAccount(FinancialAccount account) {
        ...
        if (account.getTrackingMode() != TrackingMode.FULL_LEDGER || account.getAccountKind().isLiability() ||
