...
public record LocalAuthenticationResponse(@NotNull UUID sessionId, @NotNull String accessToken, @NotNull Instant accessTokenExpiresAt,
    ...
    public static LocalAuthenticationResponse from(LocalAuthenticationResult result, Instant serverTime, String refreshToken) {
        return new LocalAuthenticationResponse(result.sessionId(), result.accessToken(), result.accessTokenExpiresAt(), result.refreshTokenExpiresAt(),
                serverTime, refreshToken);
    }
}
