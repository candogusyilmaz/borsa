...
public record TradeResponse(@NotNull UUID id, @NotNull UUID accountId, @NotNull String accountName, @NotNull UUID instrumentId,
    ...
    public static TradeResponse from(TradeReadModel model) {
        return new TradeResponse(model.id(), model.accountId(), model.accountName(), model.instrumentId(), model.instrumentSymbol(), model.instrumentName(),
                model.instrumentType(), model.currency(), model.side(), model.quantity().canonical(), model.unitPrice().canonical(),
