...
@Testcontainers
@Import(InvestingTradeHttpTest.TestOverrides.class)
...
class InvestingTradeHttpTest {
    ...
    void buySellCloseReopenAndReversalExposeCashSecurityAndPositionFacts() throws Exception {
        var owner = authenticated("investing-lifecycle-owner@example.com");
        ...
        mockMvc.perform(post("/api/v1/trades/previews").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
                .content(previewJson(accounts.brokerageId(), GLOBAL_INSTRUMENT_ID, "BUY", "1", "20", "0", "CURRENT_ACTION", BUY_AT, 0, false)))
        ...
        var buyPreview = mockMvc
                .perform(post("/api/v1/trades/previews").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
        ...
        var buyResult = mockMvc
                .perform(post("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON).content(buyJson))
        ...
        var idempotentReplay = mockMvc
                .perform(post("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(post("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(get("/api/v1/trades/{tradeId}", buyId).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CACHE_CONTROL, "no-store")).andExpect(jsonPath("$.securityPosting.quantityDelta", equalTo("10")));
        mockMvc.perform(get("/api/v1/activities/{activityId}", buyId).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk())
        ...
        var positionAfterBuy = mockMvc
                .perform(get("/api/v1/investing/positions/{accountId}/{instrumentId}", accounts.brokerageId(), ownerInstrument)
        ...
        mockMvc.perform(get("/api/v1/reconciliations/{reconciliationId}", reconciliationId).header(HttpHeaders.AUTHORIZATION, owner.bearer()))
                .andExpect(status().isOk()).andExpect(jsonPath("$.lifecycleStatus", equalTo("STALE")));
        mockMvc.perform(get("/api/v1/accounts/{accountId}/balance", accounts.brokerageId()).param("asOf", "2026-09-19T11:05:00Z")
                .header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk()).andExpect(jsonPath("$.ledgerBalance", equalTo("457")));
        ...
        mockMvc.perform(
                get("/api/v1/investing/positions").header(HttpHeaders.AUTHORIZATION, owner.bearer()).param("accountId", accounts.brokerageId().toString()))
                .andExpect(status().isOk()).andExpect(jsonPath("$.items.length()", equalTo(0)));
        mockMvc.perform(get("/api/v1/investing/positions/{accountId}/{instrumentId}", accounts.brokerageId(), ownerInstrument).header(HttpHeaders.AUTHORIZATION,
                owner.bearer())).andExpect(status().isOk()).andExpect(jsonPath("$.quantity", equalTo("0")))
                .andExpect(jsonPath("$.remainingEconomicBasis", equalTo("0")));
        ...
        var reversal = mockMvc
                .perform(post("/api/v1/activities/{activityId}/reversals", reopenId).header(HttpHeaders.AUTHORIZATION, owner.bearer())
                        .contentType(MediaType.APPLICATION_JSON).content(reversalJson(uuid("61000000-0000-4000-8000-000000000005"), "Incorrect reopen")))
        ...
        mockMvc.perform(get("/api/v1/trades/{tradeId}", reopenId).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/v1/investing/positions/{accountId}/{instrumentId}", accounts.brokerageId(), ownerInstrument).header(HttpHeaders.AUTHORIZATION,
                owner.bearer())).andExpect(status().isOk()).andExpect(jsonPath("$.quantity", equalTo("0")))
                .andExpect(jsonPath("$.remainingEconomicBasis", equalTo("0")));
        mockMvc.perform(post("/api/v1/activities/{activityId}/reversals", reopenId).header(HttpHeaders.AUTHORIZATION, owner.bearer())
                .contentType(MediaType.APPLICATION_JSON).content(reversalJson(uuid("61000000-0000-4000-8000-000000000006"), "Second reversal")))
                .andExpect(status().isConflict()).andExpect(jsonPath("$.code", equalTo("ACTIVITY_ALREADY_REVERSED")));

        var otherOwner = authenticated("investing-lifecycle-other@example.com");
        mockMvc.perform(get("/api/v1/trades/{tradeId}", buyId).header(HttpHeaders.AUTHORIZATION, otherOwner.bearer())).andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code", equalTo("TRADE_NOT_FOUND")));
        mockMvc.perform(get("/api/v1/investing/positions/{accountId}/{instrumentId}", accounts.brokerageId(), ownerInstrument).header(HttpHeaders.AUTHORIZATION,
                otherOwner.bearer())).andExpect(status().isNotFound()).andExpect(jsonPath("$.code", equalTo("POSITION_NOT_FOUND")));

        var history = mockMvc
                .perform(get("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).param("accountId", accounts.brokerageId().toString())
                        .param("instrumentId", ownerInstrument.toString()))
                .andExpect(status().isOk()).andExpect(header().string(HttpHeaders.CACHE_CONTROL, "no-store"))
                .andExpect(jsonPath("$.items.length()", equalTo(4))).andExpect(jsonPath("$.size", equalTo(50))).andReturn();
        assertSliceShape(history);
        var activityList = mockMvc
                .perform(get("/api/v1/activities").param("accountId", accounts.brokerageId().toString()).header(HttpHeaders.AUTHORIZATION, owner.bearer()))
    ...
    void fundedEtfTradePreviewCommitAndReadsExposeCashSecurityAndPositionEffects() throws Exception {
        var owner = authenticated("investing-etf-trade-owner@example.com");
        ...
        var tradeRead = mockMvc.perform(get("/api/v1/trades/{tradeId}", tradeId).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/v1/investing/positions/{accountId}/{instrumentId}", accounts.brokerageId(), instrumentId).header(HttpHeaders.AUTHORIZATION,
                owner.bearer())).andExpect(status().isOk()).andExpect(jsonPath("$.instrumentType", equalTo("ETF")))
                .andExpect(jsonPath("$.quantity", equalTo("3"))).andExpect(jsonPath("$.remainingEconomicBasis", equalTo("38.02")))
                .andExpect(jsonPath("$.calculationPolicy", equalTo("WEIGHTED_AVERAGE_ECONOMIC_V1")))
                .andExpect(jsonPath("$.projectionStatus", equalTo("CURRENT")));
        mockMvc.perform(get("/api/v1/accounts/{accountId}/balance", accounts.brokerageId()).header(HttpHeaders.AUTHORIZATION, owner.bearer()))
                .andExpect(status().isOk()).andExpect(jsonPath("$.ledgerBalance", equalTo("461.98")));
    }

    @Test
    void historicalNegativeCashAndInactiveInstrumentCloseFollowRecordingRules() throws Exception {
        var owner = authenticated("investing-historical-owner@example.com");
        ...
        var updatedInstrument = mockMvc.perform(put("/api/v1/reference/instruments/{instrumentId}", instrumentId)
                .header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON).content("""
    ...
    void ownerInstrumentVisibilityCurrencyAndUnsupportedTypeAreEnforced() throws Exception {
        var owner = authenticated("investing-visibility-owner@example.com");
        var otherOwner = authenticated("investing-visibility-other@example.com");
    ...
    void currentBuyUsesSoftFloorConfirmationBeforeRecordingNegativeCash() throws Exception {
        var owner = authenticated("investing-soft-floor-owner@example.com");
    ...
    void halfEvenMidpointGrossSurvivesPostgresPersistenceAndTradeReads() throws Exception {
        var owner = authenticated("investing-half-even-owner@example.com");
        ...
        mockMvc.perform(get("/api/v1/trades/{activityId}", idFrom(committed)).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk())
    ...
    void backdatedAndSameTimeTradesReplayByExplicitOrderAndReadSlicesStayDeterministic() throws Exception {
        var owner = authenticated("investing-backdated-owner@example.com");
        ...
        mockMvc.perform(
                get("/api/v1/investing/positions/{accountId}/{instrumentId}", brokerageId, instrumentId).header(HttpHeaders.AUTHORIZATION, owner.bearer()))
        ...
        var newestTrades = mockMvc
                .perform(get("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).param("accountId", brokerageId.toString())
                        .param("instrumentId", instrumentId.toString()).param("page", "0").param("size", "2"))
                ...
                .andReturn();
        assertSliceShape(newestTrades);
        var olderTrades = mockMvc
                .perform(get("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).param("accountId", brokerageId.toString())
                        .param("instrumentId", instrumentId.toString()).param("page", "1").param("size", "2"))
        ...
        assertSliceShape(olderTrades);
        mockMvc.perform(get("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).param("instrumentId", GLOBAL_INSTRUMENT_ID.toString()))
                .andExpect(status().isOk()).andExpect(jsonPath("$.items.length()", equalTo(1)))
                .andExpect(jsonPath("$.items[0].instrumentId", equalTo(GLOBAL_INSTRUMENT_ID.toString())));

        var openPositions = mockMvc
                .perform(get("/api/v1/investing/positions").header(HttpHeaders.AUTHORIZATION, owner.bearer()).param("accountId", brokerageId.toString())
                        .param("page", "0").param("size", "1"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.items.length()", equalTo(1))).andExpect(jsonPath("$.hasNext", equalTo(true))).andReturn();
        assertSliceShape(openPositions);
        mockMvc.perform(get("/api/v1/investing/positions").header(HttpHeaders.AUTHORIZATION, owner.bearer()).param("accountId", brokerageId.toString())
        ...
        var otherOwner = authenticated("investing-backdated-other@example.com");
        mockMvc.perform(get("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, otherOwner.bearer())).andExpect(status().isOk())
                .andExpect(jsonPath("$.items.length()", equalTo(0)));
        mockMvc.perform(get("/api/v1/investing/positions").header(HttpHeaders.AUTHORIZATION, otherOwner.bearer())).andExpect(status().isOk())
                .andExpect(jsonPath("$.items.length()", equalTo(0)));
    }

    @Test
    void precisionFutureEconomicOrderOversellAndStaleVersionsReturnStableProblems() throws Exception {
        var owner = authenticated("investing-trade-validation-owner@example.com");
        ...
        var firstPreview = mockMvc
                ...
                .andReturn();
        mockMvc.perform(
                post("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
        ...
        var holdingsOnly = mockMvc.perform(
                post("/api/v1/accounts").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON).content(holdingsOnlyJson))
                .andExpect(status().isCreated()).andReturn();
        mockMvc.perform(post("/api/v1/trades/previews").header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
    ...
    void cashProjectionOverflowReturnsStablePrecisionProblemWithoutWritingTradeFacts() throws Exception {
        var owner = authenticated("investing-cash-overflow-owner@example.com");
    ...
    void reversingBuyThatSupportsALaterSaleRollsBackAllTradeFacts() throws Exception {
        var owner = authenticated("investing-invalid-reversal-owner@example.com");
        ...
        mockMvc.perform(post("/api/v1/activities/{activityId}/reversals", buyId).header(HttpHeaders.AUTHORIZATION, owner.bearer())
    ...
    private Accounts fundedBrokerage(Identity identity, String label, String amount) throws Exception {
        ...
        mockMvc.perform(post("/api/v1/transfers").header(HttpHeaders.AUTHORIZATION, identity.bearer()).contentType(MediaType.APPLICATION_JSON)
    ...
    private UUID createInstrument(Identity identity, String symbol, String type, String currency) throws Exception {
        var result = mockMvc.perform(post("/api/v1/reference/instruments").header(HttpHeaders.AUTHORIZATION, identity.bearer())
                .contentType(MediaType.APPLICATION_JSON).content(instrumentJson(symbol, type, currency))).andExpect(status().isCreated()).andReturn();
        return idFrom(result);
    }
    ...
    private MvcResult commitTrade(Identity identity, UUID requestId, UUID accountId, UUID instrumentId, String side, String quantity, String unitPrice,
        ...
        var preview = mockMvc
                .perform(post("/api/v1/trades/previews").header(HttpHeaders.AUTHORIZATION, identity.bearer()).contentType(MediaType.APPLICATION_JSON)
                        .content(previewJson(accountId, instrumentId, side, quantity, unitPrice, commission, "CURRENT_ACTION", effectiveAt, sequence, false)))
                .andExpect(status().isOk()).andReturn();
        return mockMvc
                .perform(post("/api/v1/trades").header(HttpHeaders.AUTHORIZATION, identity.bearer()).contentType(MediaType.APPLICATION_JSON)
    ...
    private UUID reconcile(Identity identity, UUID accountId, int closingBalance) throws Exception {
        ...
        mockMvc.perform(post("/api/v1/accounts/{accountId}/reconciliation-previews", accountId).header(HttpHeaders.AUTHORIZATION, identity.bearer())
        ...
        var result = mockMvc
                .perform(post("/api/v1/accounts/{accountId}/reconciliations", accountId).header(HttpHeaders.AUTHORIZATION, identity.bearer())
                        .contentType(MediaType.APPLICATION_JSON).content(commitJson))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.lifecycleStatus", equalTo("CURRENT"))).andReturn();
        return idFrom(result);
    }

    private Identity authenticated(String email) {
        var userId = registrationService.register(email, PASSWORD);
        var session = sessionIssuanceService.issue(userId, "investing-trade-http-test");
