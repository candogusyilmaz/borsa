...
class TradeImportHttpTest {
    ...
    void authenticatedMultipartUploadPreviewCommitAndProvenanceAreExposedWithoutCaching() throws Exception {
        var owner = authenticated("trade-import-http-owner@example.com");
        ...
        mockMvc.perform(multipart("/api/v1/imports").file(new MockMultipartFile("file", "trades.csv", "text/csv", "wrong-header".getBytes()))
                .param("clientRequestId", UUID.randomUUID().toString()).param("accountId", accountId.toString())
                .header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isUnprocessableContent())
                .andExpect(jsonPath("$.code", equalTo("IMPORT_FILE_FORMAT_INVALID")));
        ...
        mockMvc.perform(get(previewUrl).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CACHE_CONTROL, "no-store")).andExpect(jsonPath("$.commitEligible", equalTo(true)))
                .andExpect(jsonPath("$.rows[0].externalId", equalTo("source-1"))).andExpect(jsonPath("$.summary.cashBalanceBefore", equalTo("500")))
                .andExpect(jsonPath("$.summary.cashBalanceAfter", equalTo("479")));
        var previewResult = mockMvc.perform(get(previewUrl).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andReturn();
        var previewToken = JsonPath.<String>read(previewResult.getResponse().getContentAsString(), "$.previewToken");

        var otherOwner = authenticated("trade-import-http-other@example.com");
        mockMvc.perform(get(previewUrl).header(HttpHeaders.AUTHORIZATION, otherOwner.bearer())).andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code", equalTo("IMPORT_NOT_FOUND")));
        mockMvc.perform(
                post("/api/v1/imports/{batchId}/commit", batchId).header(HttpHeaders.AUTHORIZATION, otherOwner.bearer()).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"clientRequestId\":\"" + UUID.randomUUID() + "\",\"previewToken\":\"" + previewToken + "\"}"))
                .andExpect(status().isNotFound()).andExpect(jsonPath("$.code", equalTo("IMPORT_NOT_FOUND")));
        ...
        mockMvc.perform(multipart("/api/v1/imports").file(uploadFile).param("clientRequestId", UUID.randomUUID().toString())
                .param("accountId", foreignAccountId.toString()).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isNotFound());
        ...
        var duplicateResult = mockMvc
                .perform(multipart("/api/v1/imports").file(uploadFile).param("clientRequestId", UUID.randomUUID().toString())
                        .param("accountId", accountId.toString()).header(HttpHeaders.AUTHORIZATION, owner.bearer()))
        ...
        var commitResult = mockMvc
                .perform(post("/api/v1/imports/{batchId}/commit", batchId).header(HttpHeaders.AUTHORIZATION, owner.bearer())
        ...
        mockMvc.perform(post("/api/v1/imports/{batchId}/commit", batchId).header(HttpHeaders.AUTHORIZATION, owner.bearer())
                .contentType(MediaType.APPLICATION_JSON).content(commitBody)).andExpect(status().isOk())
                .andExpect(jsonPath("$.activityIds[0]", equalTo(tradeId.toString())));
        mockMvc.perform(
                post("/api/v1/imports/{batchId}/commit", batchId).header(HttpHeaders.AUTHORIZATION, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"clientRequestId\":\"" + UUID.randomUUID() + "\",\"previewToken\":\"" + previewToken + "\"}"))
                .andExpect(status().isConflict()).andExpect(jsonPath("$.code", equalTo("IMPORT_ALREADY_COMMITTED")));
        mockMvc.perform(get("/api/v1/trades/{tradeId}", tradeId).header(HttpHeaders.AUTHORIZATION, owner.bearer())).andExpect(status().isOk())
        ...
        var history = mockMvc.perform(get("/api/v1/trades").param("accountId", accountId.toString()).header(HttpHeaders.AUTHORIZATION, owner.bearer()))
                .andExpect(status().isOk()).andExpect(header().string(HttpHeaders.CACHE_CONTROL, "no-store")).andReturn();
    ...
    void malformedAndOversizedMultipartInputsUseSafeErrors() throws Exception {
        var owner = authenticated("trade-import-http-errors@example.com");
        var accountId = createBrokerage(owner.userId());
        var auth = HttpHeaders.AUTHORIZATION;
        mockMvc.perform(multipart("/api/v1/imports").param("clientRequestId", UUID.randomUUID().toString()).param("accountId", accountId.toString())
                .header(auth, owner.bearer())).andExpect(status().isUnprocessableContent());
        mockMvc.perform(multipart("/api/v1/imports").file(new MockMultipartFile("file", "empty.csv", "text/csv", new byte[0]))
                .param("clientRequestId", UUID.randomUUID().toString()).param("accountId", accountId.toString()).header(auth, owner.bearer()))
        ...
        mockMvc.perform(post("/api/v1/imports").contentType(MediaType.APPLICATION_JSON).content("{}").header(auth, owner.bearer()))
        ...
        var previewResponse = mockMvc.perform(get("/api/v1/imports/{batchId}/preview", validBatchId).header(auth, owner.bearer())).andExpect(status().isOk())
                .andReturn();
        var previewToken = JsonPath.<String>read(previewResponse.getResponse().getContentAsString(), "$.previewToken");
        mockMvc.perform(post("/api/v1/imports/{batchId}/commit", validBatchId).header(auth, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(post("/api/v1/imports/{batchId}/commit", invalidBatchId).header(auth, owner.bearer()).contentType(MediaType.APPLICATION_JSON)
    ...
    private Identity authenticated(String email) {
        ...
        var session = sessionIssuanceService.issue(userId, "trade-import-http-test");
