...
class GlobalExceptionHandlerIntegrationTest {
    ...
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(new ErrorHandlingTestController())
                ...
                .addFilters(new RequestTraceFilter(() -> UUID.fromString("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"))).build();
    }

    @Test
    void applicationExceptionProducesProblemDetailAndCorrelatesTraceId() throws Exception {
        var result = mockMvc.perform(get("/test/errors/application").header(RequestTraceFilter.TRACE_ID_HEADER, "client-supplied"))
                .andExpect(status().isNotFound()).andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON))
                .andExpect(jsonPath("$.type").value("https://canverse.dev/problems/entity-not-found")).andExpect(jsonPath("$.title").value("Not Found"))
                .andExpect(jsonPath("$.status").value(404)).andExpect(jsonPath("$.instance").value("/test/errors/application"))
                .andExpect(jsonPath("$.code").value("ENTITY_NOT_FOUND")).andExpect(jsonPath("$.key").value("error.common.entity_not_found"))
                .andExpect(jsonPath("$.params.entity").value("Example")).andExpect(jsonPath("$.params.id").value("00000000-0000-0000-0000-000000000001"))
                .andExpect(jsonPath("$.timestamp").value(TEST_TIME.toString())).andExpect(header().exists(RequestTraceFilter.TRACE_ID_HEADER)).andReturn();
    ...
    void internalApplicationExceptionDoesNotExposeParamsOrMessage() throws Exception {
        var response = mockMvc.perform(get("/test/errors/internal")).andExpect(status().isInternalServerError())
                .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON)).andExpect(jsonPath("$.code").value("INTERNAL_ERROR"))
                .andExpect(jsonPath("$.key").value("error.common.internal_error")).andExpect(jsonPath("$.params").doesNotExist())
                .andExpect(jsonPath("$.detail").doesNotExist()).andReturn().getResponse().getContentAsString();
    ...
    void unhandledExceptionDoesNotExposeInternalDetails() throws Exception {
        var response = mockMvc.perform(get("/test/errors/unexpected")).andExpect(status().isInternalServerError())
                .andExpect(jsonPath("$.code").value("INTERNAL_ERROR")).andExpect(jsonPath("$.params").doesNotExist())
                .andExpect(jsonPath("$.detail").doesNotExist()).andReturn().getResponse().getContentAsString();
    ...
    void malformedJsonUsesStableCodeWithoutParserMessage() throws Exception {
        var response = mockMvc.perform(post("/test/errors/validation").contentType(MediaType.APPLICATION_JSON).content("{\"email\":"))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("MALFORMED_REQUEST")).andReturn().getResponse().getContentAsString();
    ...
    void missingRequestParameterUsesSafeParameterName() throws Exception {
        mockMvc.perform(get("/test/errors/missing")).andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("MISSING_REQUEST_VALUE"))
                .andExpect(jsonPath("$.params.parameter").value("parameter"));
    }

    @Test
    void methodNotAllowedUsesStableCode() throws Exception {
        mockMvc.perform(post("/test/errors/application")).andExpect(status().isMethodNotAllowed()).andExpect(jsonPath("$.code").value("METHOD_NOT_ALLOWED"));
    }

    @Test
    void unsupportedMediaTypeUsesStableCode() throws Exception {
        mockMvc.perform(post("/test/errors/media").contentType(MediaType.TEXT_PLAIN).content("not-json")).andExpect(status().isUnsupportedMediaType())
                .andExpect(jsonPath("$.code").value("UNSUPPORTED_MEDIA_TYPE"));
    }

    @Test
    void unexpectedPersistenceExceptionUsesGenericServerError() throws Exception {
        var response = mockMvc.perform(get("/test/errors/conflict")).andExpect(status().isInternalServerError())
                .andExpect(jsonPath("$.code").value("INTERNAL_ERROR")).andExpect(jsonPath("$.params").doesNotExist()).andReturn().getResponse()
                .getContentAsString();
    ...
    void knownPersistenceConstraintUsesTheCapabilityErrorCodeWithoutLeakingDatabaseDetails() throws Exception {
        var response = mockMvc.perform(get("/test/errors/known-conflict")).andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("EMAIL_ALREADY_REGISTERED")).andExpect(jsonPath("$.key").value("error.identity.email_already_registered"))
                .andExpect(jsonPath("$.params").doesNotExist()).andReturn().getResponse().getContentAsString();
    ...
    void optimisticLockFailureUsesTheExistingStateConflictContract() throws Exception {
        mockMvc.perform(get("/test/errors/optimistic")).andExpect(status().isConflict()).andExpect(jsonPath("$.code").value("STATE_CONFLICT"))
