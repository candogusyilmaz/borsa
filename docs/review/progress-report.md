...
| PR-015 — Database-backed access-token authentication | **Implemented and reviewed; ready for supervisor commit**    | One owner-scoped session/user lookup and one read-only decoded-JWT converter produce a minimal authority-free Spring JWT authentication only for a currently eligible exact pair; no bearer filter, HTTP, authorization, mutation, or schema surface |
| Automated backend coverage                         | 95 tests, all green                                          | Existing 88 tests plus 7 focused converter control-flow and PostgreSQL decoder-to-converter cases                                                                                                                                                       |

Overall status: **PR-014 is accepted in local commit `4621473`. PR-015 database-backed access-token authentication is implemented, independently reviewed with no findings, and fully verified with all 95 tests green; it remains active only until the supervisor creates its local commit. HTTP bearer filters/endpoints, authorities/owner helpers, persistent signing keys, refresh rotation/reuse response, logout/revocation, and abuse controls remain deferred.**
...
- Review/acceptance: supervisor acceptance/local commit remains pending; PR-015 remains active in `CURRENT.md` until that completion boundary.
...
|        R1 | Foundation, identity, auth, sessions and jobs                | In progress — PR-014 accepted in `4621473`; PR-015 database-backed access-token authentication implemented, independently reviewed, fully verified, and ready for supervisor commit; HTTP bearer endpoints/authorization, rotation/revocation, jobs and abuse controls deferred |
