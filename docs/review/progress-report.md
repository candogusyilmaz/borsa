...
| PR-014 — Local access-token decoding               | **Implemented and reviewed; ready for supervisor commit**    | One production decoder over the existing local RSA public key plus strict RS256/header/claim/time-envelope validation; no bearer filter, principal, HTTP, persistence, or key-management surface                                                  |
| Automated backend coverage                         | 88 tests, all green                                          | Existing 84 tests plus 4 focused real-encoder/decoder configuration and validation cases                                                                                                                                                              |

Overall status: **PR-013 is accepted in local commit `7bb7c40`. PR-014 production local access-token decoding is implemented, independently reviewed with no remaining findings, and fully verified with all 88 tests green; it remains active only until the supervisor creates its local commit. HTTP token delivery, bearer filters/principals, persistent signing keys, refresh rotation/reuse response, logout/revocation, and abuse controls remain deferred.**
...
|        R1 | Foundation, identity, auth, sessions and jobs                | In progress — PR-013 accepted in `7bb7c40`; PR-014 local access-token decoding implemented, independently reviewed, fully verified, and ready for supervisor commit; HTTP token delivery, bearer principal/filter chain, rotation/revocation, jobs and abuse controls deferred |
