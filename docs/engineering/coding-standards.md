...
- The current process-local authentication abuse limiter is sufficient for the single-process deployment. Before multi-instance deployment or broader limiter expansion, evaluate maintained distributed or cluster-safe rate-limiting support against the required semantics; do not assume process-local state scales.
