...
Status: **IMPLEMENTATION AND REVIEW COMPLETE - AWAITING USER COMMIT**
...
Spotless, the 29-test focused suite, the full 105-test suite, and `verify` pass with no failures, errors, or skipped tests. Independent correction-cycle review fixed the exact cookie `Expires` value and passed with no remaining `MUST FIX` or `SHOULD FIX` findings. The user-owned accepting commit remains pending.
...
PR-017 additionally adds `identity.input.LocalLoginRequest`, `identity.input.RefreshTokenDelivery`, `identity.output.LocalLoginResponse`, and `identity.web.LocalLoginController`.
...
- The versioned local-registration endpoint remains intentionally public under the PR-016 security chain: it delegates once to the PR-005 transactional workflow and returns only the new user ID. PR-017 now adds the separate initial-login boundary; refresh consumption and lifecycle behavior remain deferred.
...
PR-017's implementation and review are complete, but its user-owned accepting commit is not present in the inspected repository. PR-018 must record that real commit hash before production implementation begins. See `CURRENT.md` and `PR-018-refresh-session-rotation-and-http-refresh.md`; do not invent the prerequisite hash or broaden the active scope.
...
- HTTP bearer authentication is installed for `/api/v1/**`, and PR-017's initial login/delivery boundary is implemented and reviewed; no production refresh endpoint/rotation, endpoint authorization, roles/permissions, or owner helpers exist yet. PR-018 specifies the refresh vertical slice but is not implemented.
- No refresh rotation/replacement or family-reuse response exists in production yet; logout, explicit revocation, and session listing/deletion remain outside PR-018. Read-only opaque credential authentication does not itself authorize a mutation.
