...
- The versioned local-registration endpoint remains intentionally public under the PR-016 security chain: it delegates once to the PR-005 transactional workflow and returns only the new user ID; production login and refresh-token delivery remain deferred.
...
- HTTP bearer authentication now has one servlet-only stateless `/api/v1/**` chain: the exact registration POST is public, all other matched requests use the accepted decoder and database-backed authority-free converter, request tracing precedes security, and expected authentication failures share the existing safe `INVALID_CREDENTIALS` Problem Detail.

## Active implementation unit

PR-016 HTTP bearer authentication boundary is implemented, independently reviewed after correction cycle 1 with no remaining findings, fully verified, and ready for the supervisor's local commit. It installs the first stateless `/api/v1/**` security filter chain over the accepted decoder and current-session converter without adding a production endpoint or token-delivery policy.

See `CURRENT.md` and `PR-016-http-bearer-authentication-boundary.md` for the authoritative active scope.

## Next likely implementation areas

These are planning hints for work after PR-016, not active specifications.

Likely sequence:

1. HTTP login/token delivery after resolving the distinct web-cookie and native-response refresh-token transport requirements;
2. refresh-session rotation and family-reuse response after resolving lifetime/reuse-policy choices;
3. owner/principal helpers and endpoint authorization over the accepted authenticated identity;
...
- HTTP bearer authentication is installed for `/api/v1/**`; no production login or refresh-token delivery route, endpoint authorization, roles/permissions, or owner helpers exist yet.
