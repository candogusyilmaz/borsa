...
PR-007 exposes the completed atomic local-registration workflow through one versioned HTTP endpoint. It is an HTTP-boundary slice only and does not add login, tokens, sessions, or Spring Security.

See `CURRENT.md` and `PR-007-http-local-account-registration.md` for the authoritative active scope.

## Next likely implementation areas

These are planning hints for work after PR-007, not active specifications.

Likely sequence:

1. local password-login application behavior;
...
The exact next behavioral implementation unit must be designed just-in-time after PR-007 is reviewed and accepted.
