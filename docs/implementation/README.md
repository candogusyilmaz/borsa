...
Implementation and review agents read `CURRENT.md`, then the active specification completely. The active specification is execution-self-contained; agents do not preload long-lived design, review, or contract documents, consulting them only when a concrete ambiguity or unstated invariant requires it. Planning agents start with `STATE.md`, `CURRENT.md`, this workflow document when needed, and `PR-TEMPLATE.md`, then load only the roadmap and domain sections required to define the next bounded unit, producing an execution-self-contained specification. Historical PR specifications are not normal bootstrap context.
...
A PR should be a substantial, coherent capability or lifecycle slice carried through every layer needed for a meaningful testable outcome. Combine tightly coupled database, domain, repository, application, API, security, and test work when splitting them would leave an incomplete or mechanical boundary. An explicitly authorized governing-document unit may have a documentation/planning-only boundary before implementation cleanup, but it must not start later cleanup or perform mechanical all-layer work. Split independent business capabilities, invariants, migration risks, security boundaries, or scopes that no longer fit one careful focused review. Do not pad a PR with unrelated cleanup or speculative infrastructure; sizing is determined by capability and review coherence, not a fixed line or file count. Context optimization is achieved through execution-self-contained specifications, targeted source reads, and bounded command output, not by breaking coherent features into artificial micro-PRs.

Use [PR-TEMPLATE.md](PR-TEMPLATE.md) for each new specification. Specifications must be execution-self-contained: inlining exact invariants, formulas, schema details, API contracts, error codes, and acceptance criteria required for implementation, while referencing authoritative long-lived documents for provenance rather than duplicating unrelated architecture documentation.
