...
import { AsOfDateDrawer } from './as-of-date-drawer';
...
export function BalanceLiquidityCard({ account, expanded, onToggle }: BalanceLiquidityCardProps) {
  ...
  const [asOfDrawerOpened, { open: openAsOfDrawer, close: closeAsOfDrawer }] = useDisclosure(false);
  ...
  return (
    <>
                ...
                <div className={classes.collapsedMetricItem}>
                  ...
                  <span className={classes.collapsedMetricValue}>
                    {balanceQuery.isLoading ? (
                      ...
                      formatCurrency(balance?.overdraftUsed, account.currency)
                    )}
                  </span>
                </div>
              </div>
      ...
      {/* As-Of Date Bottom Sheet Drawer */}
      <AsOfDateDrawer
        account={account}
        opened={asOfDrawerOpened}
        onClose={closeAsOfDrawer}
