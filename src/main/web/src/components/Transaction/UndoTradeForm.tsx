...
function DetailLine({label, value, color}: { label: string; value: React.ReactNode; color?: string }) {
    return (
    ...
    return (
        <Stack>
            ...
            <Alert title="Warning" color="red" variant="filled">
                This will permanently remove this trade record. This action cannot be reversed. Daily portfolio
                snapshots will not be retroactively
                modified.
            </Alert>

            <Group>
                <Checkbox label="I understand this action is permanent"
                          onChange={(e) => setHasConfirmed(e.currentTarget.checked)}/>
            </Group>
            ...
            <Group justify="flex-end">
                ...
                <Button onClick={handleFormSubmit} color="red" loading={mutation.isPending}
                        disabled={!hasConfirmed || mutation.isSuccess}>
                    Confirm Removal
                </Button>
            </Group>
        </Stack>
