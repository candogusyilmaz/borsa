...
export function HoldingsTable() {
    ...
    const {data, status} = useQuery(
        queries.portfolio.fetchPortfolio(includeCommission)
    );
    ...
    return (
        <Inner
            ...
            onToggleCommission={setIncludeCommission}
        />
    );
}
...
function Inner({
    ...
    const columns = [
        ...
        columnHelper.display({
            ...
            header: "Portfolio %",
            cell: ({row}) =>
                format.toLocalePercentage((row.original.value / totalValue) * 100),
        }),
    ];
    ...
    const totalDailyChangePercent =
        totals.value - totals.dailyProfit !== 0
            ? (totals.dailyProfit / (totals.value - totals.dailyProfit)) * 100
            : 0;
    ...
    return (
        <Stack>
                    ...
                    >
                        ...
                        <Table.Tbody>
                            {table.getRowModel().rows.map((row) => (
                                <Table.Tr key={row.id}>
                                    {row.getVisibleCells().map((cell) => (
                                        <Table.Td
                                            key={cell.id}
                                            ta={cell.column.id === "symbol" ? "left" : "right"}
                                        >
                                            {flexRender(
                            ...
                            >
                                ...
                                <Table.Td
                                    ta="right"
                                    c={totals.dailyProfit >= 0 ? "teal" : "red"}
                                >
                                    {format.toCurrency(totals.dailyProfit)}
                                </Table.Td>
                                <Table.Td
                                    ta="right"
                                    c={totalDailyChangePercent >= 0 ? "teal" : "red"}
                                >
                                ...
                                <Table.Td
                                    ta="right"
                                    c={totalProfitPercentage >= 0 ? "teal" : "red"}
                                >
            ...
            <Text c="dimmed" size="xs">
                * Last prices are updated every week day at 19:00.
            </Text>
        </Stack>
    );
}
