...
public record InstrumentSummaryResponse(@NotNull UUID id, @NotNull String symbol, @NotNull String name, @NotNull InstrumentType instrumentType,
    ...
    public static InstrumentSummaryResponse from(ReferenceCatalogReadRepository.InstrumentView view) {
        ...
        return new InstrumentSummaryResponse(row.id(), row.symbol(), row.name(), row.instrumentType(), row.marketId(), row.marketCode(),
                row.quotationCurrency(), row.valuationMethod(), row.active(), row.sourceKind(), row.ownerId() != null, aliases);
    }
}
