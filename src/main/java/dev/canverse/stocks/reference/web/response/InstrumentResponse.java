...
public record InstrumentResponse(@NotNull UUID id, UUID ownerId, @NotNull UUID marketId, @NotNull String marketCode, @NotNull String symbol,
    ...
    public static InstrumentResponse from(ReferenceCatalogReadRepository.InstrumentView view) {
        ...
        return new InstrumentResponse(row.id(), row.ownerId(), row.marketId(), row.marketCode(), row.symbol(), row.name(), row.instrumentType(),
                row.quotationCurrency(), row.valuationMethod(), row.active(), row.sourceKind(), row.version(), row.createdAt(), row.updatedAt(), aliases);
    }
}
