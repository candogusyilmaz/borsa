...
public class StatisticsRepository {
    ...
    public RealizedGains getRealizedGains(
            long userId, String periodType, List<Long> portfolioIds, String targetCurrency) {
        ...
        return jdbcTemplate.query(
                ...
                rs -> {
                    rs.next();
                    return new RealizedGains(
                            rs.getBigDecimal("cpg"),
                            rs.getBigDecimal("ppg"),
                            Calculator.calculatePercentageChange(
                                    rs.getBigDecimal("cpg"), rs.getBigDecimal("ppg")),
                            targetCurrency);
                });
    }
    ...
    private String getPeriodCondition(String periodType, boolean isCurrent) {
        ...
        return switch (periodType) {
            ...
            case "week" ->
                    "DATE_TRUNC('week', t.action_date) = DATE_TRUNC('week', CURRENT_DATE"
                            + interval
                            + ")";
            case "month" ->
                    "DATE_TRUNC('month', t.action_date) = DATE_TRUNC('month', CURRENT_DATE"
                            + interval
                            + ")";
            case "year" ->
                    "DATE_TRUNC('year', t.action_date) = DATE_TRUNC('year', CURRENT_DATE"
                            + interval
                            + ")";
        ...
        return jdbcTemplate.query(
                ...
                rs -> {
                    ...
                    var percentageChange =
                            Calculator.calculatePercentageChange(currentValue, previousCloseValue);
    ...
    public TotalBalance getTotalBalance(
            long userId, List<Long> portfolioIds, String targetCurrency) {
        if (portfolioIds.isEmpty()) {
                ...
                rs -> {
                    if (!rs.next()) {
                        return new TotalBalance(
                                BigDecimal.ZERO, BigDecimal.ZERO, null, targetCurrency);
                    }
