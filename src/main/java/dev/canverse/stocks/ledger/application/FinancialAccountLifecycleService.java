...
public class FinancialAccountLifecycleService {
    ...
    public FinancialAccountResponse archive(UUID ownerUserAccountId, UUID accountId, ArchiveAccountRequest request) {
        ...
        var hash =
                fingerprint.hash(fingerprint.values("accountId", accountId.toString(), "version", request.version()));
        ...
        var replay = idempotencyStore.replay(
                ...
                ownerUserAccountId,
                LedgerCommandScopes.ACCOUNT_ARCHIVE,
                hash,
    ...
    @Transactional
    public FinancialAccountResponse correctOpening(
        ...
        var replay = idempotencyStore.replay(
                request.clientRequestId(),
                ownerUserAccountId,
                LedgerCommandScopes.OPENING_CORRECTION,
                hash,
                FinancialAccountResponse.class);
        if (replay != null) {
            return replay;
        ...
        var original = activityRepository
                .findOwnedForUpdate(account.getCurrentOpeningActivityId(), ownerUserAccountId)
        ...
        var replacement = writeOpeningCorrection(
                ownerUserAccountId,
                account,
                original,
                originalPostings,
    ...
    private Activity writeOpeningCorrection(
            ...
            FinancialAmount replacementAmount,
            String correctionReason,
            UUID clientRequestId,
            Instant observedAt) {
        var inverse = Activity.reversal(
                idGenerator.next(),
                ownerUserAccountId,
                clientRequestId,
                LedgerCommandScopes.OPENING_CORRECTION,
        ...
        for (var originalPosting : originalPostings) {
            postingRepository.save(MoneyPosting.reversal(
                    ...
                    FinancialAmount.of(originalPosting.getAmount()).negate(),
                    observedAt));
        }

        var replacementDecision =
                replacementAmount.isNegative() ? PolicyDecision.HISTORICAL_BREACH_RECORDED : PolicyDecision.ALLOWED;
        var replacement = Activity.correctedOpeningBalance(
                idGenerator.next(),
                ownerUserAccountId,
                clientRequestId,
                LedgerCommandScopes.OPENING_CORRECTION,
        ...
        var projection = projectionRepository
                .findOwnedForUpdate(ownerUserAccountId, account.getId())
                .orElseThrow(() -> new AppException(LedgerErrorCode.OPENING_STATE_CONFLICT));
