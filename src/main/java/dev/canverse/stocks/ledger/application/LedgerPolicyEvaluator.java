    ...
    static PolicyEvaluation evaluate(
        ...
        if (recordingMode == RecordingMode.HISTORICAL_FACT) {
            return new PolicyEvaluation(
                    true,
                    after.isNegative() ? PolicyDecision.HISTORICAL_BREACH_RECORDED : PolicyDecision.ALLOWED,
                    null);
        }
        ...
        if (policy == NegativeBalancePolicy.AUTHORIZED_LIMIT) {
            ...
            if (after.compareTo(limit.negate()) < 0) {
                return new PolicyEvaluation(
                        false, PolicyDecision.NOT_APPLICABLE, LedgerErrorCode.ACCOUNT_LIMIT_EXCEEDED);
            }
            return new PolicyEvaluation(true, PolicyDecision.ALLOWED, null);
        }
        if (policy == NegativeBalancePolicy.SOFT_FLOOR && !confirmPolicyBreach) {
            return new PolicyEvaluation(
                    false, PolicyDecision.NOT_APPLICABLE, LedgerErrorCode.POLICY_BREACH_CONFIRMATION_REQUIRED);
        }
        return new PolicyEvaluation(
                true,
                policy == NegativeBalancePolicy.SOFT_FLOOR
                        ? PolicyDecision.CONFIRMED_BREACH
