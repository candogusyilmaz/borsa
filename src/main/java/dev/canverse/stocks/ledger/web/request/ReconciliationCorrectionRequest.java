...
public record ReconciliationCorrectionRequest(
    ...
    public void validate() {
        ...
        if (resolution == ReconciliationAction.CREATE_ADJUSTMENT
            ...
            throw ValidationErrors.invalidField(
                    "adjustmentReason",
                    "error.fields.ledger.adjustment_reason_required",
        ...
        if (resolution == ReconciliationAction.CONFIRM_BALANCED && adjustmentReason != null) {
            throw ValidationErrors.invalidField(
                    "adjustmentReason",
                    "error.fields.ledger.adjustment_reason_prohibited",
                    "An adjustment reason is only valid when creating an adjustment.");
        }
        if (correctionReason != null && correctionReason.trim().isEmpty()) {
            throw ValidationErrors.invalidField(
                    "correctionReason", "error.fields.common.not_blank", "The correction reason must not be blank.");
        }
    }
}
