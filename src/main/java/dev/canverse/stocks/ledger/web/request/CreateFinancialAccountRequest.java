...
public record CreateFinancialAccountRequest(
        ...
        @NotNull @JsonAlias("accountKind") AccountKind kind,
        @NotNull TrackingMode trackingMode,
        @NotBlank @Pattern(regexp = "[A-Z]{3}") String currency,
        @NotBlank @Size(max = 120) String timeZone,
        @JsonAlias({"negativeBalancePolicy", "policy"}) NegativeBalancePolicy policy,
        String authorizedLimit,
        @Valid OpeningStateRequest openingState) {

    public void validate() {
        if (trackingMode == TrackingMode.FULL_LEDGER && openingState == null) {
            throw ValidationErrors.invalidField(
                    "openingState", "error.fields.ledger.required", "A full-ledger account requires an opening state.");
        }
        if (trackingMode == TrackingMode.HOLDINGS_ONLY && openingState != null) {
            throw ValidationErrors.invalidField(
                    "openingState",
                    "error.fields.ledger.forbidden",
                    "A holdings-only account cannot have cash opening state.");
        }
    }
}
