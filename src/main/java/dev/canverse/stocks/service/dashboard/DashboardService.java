...
public class DashboardService {
    ...
    public void create(@Valid CreateDashboardRequest request) {
        var dashboard =
                new Dashboard(
                        ...
                        request.name(),
                        currencyRepository.getReferenceById(Long.valueOf(request.currencyId())));

        request.portfolioIds().stream()
                .map(Long::valueOf)
                .forEach(
                        s -> {
                            if (!portfolioRepository.existsByUserId(
                                    AuthenticationProvider.getUser().getId(), s)) {
                                throw new BadRequestException(
                                        "One or more portfolios are invalid.");
                            }
    ...
    public void delete(Long dashboardId) {
        var dashboard =
                dashboardRepository
                        .findByIdAndUserId(dashboardId, AuthenticationProvider.getUser().getId())
    ...
    public List<BasicDashboardView> getAllDashboards() {
        ...
        return qf.select(
                        Projections.constructor(
                                BasicDashboardView.class, d.id.stringValue(), d.name, d.isDefault))
                .from(d)
    ...
    public DashboardView getDefaultDashboard() {
        ...
        var dashboardId =
                qf.select(d.id)
                        ...
                        .where(
                                d.user.id
                                        .eq(AuthenticationProvider.getUser().getId())
                                        .and(d.isDefault.isTrue()))
                        .fetchOne();
    ...
    public DashboardView getDashboard(Long dashboardId) {
        ...
        var dashboard =
                qf.select(d.id.stringValue(), d.name, d.isDefault, d.currency.code)
                        .from(d)
        ...
        var portfolioIds =
                qf.select(dpf.portfolio.id)
                        .from(dpf)
                        .where(
                                dpf.dashboard
                                        .id
                                        .eq(dashboardId)
                                        .and(dpf.portfolio.archived.isFalse()))
                        .fetch();

        var realizedGains =
                statisticsRepository.getRealizedGains(
                        userId, "month", portfolioIds, dashboard.get(d.currency.code));
        var dailyChange =
                statisticsRepository.getDailyChange(
                        userId, portfolioIds, dashboard.get(d.currency.code));
        var totalBalance =
                statisticsRepository.getTotalBalance(
                        userId, portfolioIds, dashboard.get(d.currency.code));
    ...
    public List<TransactionInfo> getDashboardTransactions(Long dashboardId) {
        ...
        var currencyConvertedProfit =
                Expressions.stringTemplate(
                                "public.convert_currency({0}, {1}, {2})",
                                t.performance.profit,
                                t.position.currencyCode,
                                dashboard.getCurrency().getCode())
