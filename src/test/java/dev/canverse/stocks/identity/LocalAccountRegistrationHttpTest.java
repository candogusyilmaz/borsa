...
class LocalAccountRegistrationHttpTest {
    ...
    void successfulHttpRegistrationCommitsUserAndLocalIdentity() throws Exception {
        ...
        var result = performRegistration("Alice.Example@Example.COM", RAW_PASSWORD).andExpect(status().isCreated())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON)).andExpect(header().doesNotExist(HttpHeaders.LOCATION))
    ...
    void duplicateEmailThroughHttpReturnsSafeConflictWithoutNewRows() throws Exception {
        ...
        var result = performRegistration("ALICE.EXAMPLE@example.com", RAW_PASSWORD).andExpect(status().isConflict())
                .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON))
                .andExpect(header().string(RequestTraceFilter.TRACE_ID_HEADER, retryTraceId.toString())).andReturn();
        ...
        assertThat(response).doesNotContain("ALICE.EXAMPLE@example.com", "alice.example@example.com", RAW_PASSWORD, "uq_user_account_email_normalized",
                "uq_auth_identity_provider_subject", "constraint");
    ...
    void invalidHttpRegistrationIsRejectedBeforeIdentityWrites() throws Exception {
        assertValidationFailure("", RAW_PASSWORD, "email", "error.fields.common.not_blank", uuid("d0000000-0000-0000-0000-000000000001"));
        assertValidationFailure("not-an-email", RAW_PASSWORD, "email", "error.fields.common.email", uuid("d0000000-0000-0000-0000-000000000002"));
        assertValidationFailure(" alice@example.com", RAW_PASSWORD, "email", "error.fields.common.pattern", uuid("d0000000-0000-0000-0000-000000000003"));
        assertValidationFailure("alice@example.com ", RAW_PASSWORD, "email", "error.fields.common.pattern", uuid("d0000000-0000-0000-0000-000000000004"));
        assertValidationFailure("a".repeat(310) + "@example.com", RAW_PASSWORD, "email", "error.fields.common.size",
                uuid("d0000000-0000-0000-0000-000000000005"));
        assertValidationFailure("alice@example.com", "", "password", "error.fields.common.not_blank", uuid("d0000000-0000-0000-0000-000000000006"));
        assertValidationFailure("alice@example.com", "p".repeat(11), "password", "error.fields.common.size", uuid("d0000000-0000-0000-0000-000000000007"));
        var overlongPassword = "p".repeat(129);
        assertValidationFailure("alice@example.com", overlongPassword, "password", "error.fields.common.size", uuid("d0000000-0000-0000-0000-000000000008"));
    ...
    void malformedHttpRequestUsesSafeProblemContractWithoutWrites() throws Exception {
        ...
        var result = mockMvc
                .perform(post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).content(submittedFragment))
        ...
        assertThat(response).doesNotContain("Alice.Example@example.com", RAW_PASSWORD, "JsonParseException", "Unexpected end-of-input");
    ...
    private ResultActions performRegistration(String email, String password) throws Exception {
        return mockMvc.perform(
                post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).content(requestJson(email, password)));
    }

    private void assertValidationFailure(String email, String password, String expectedField, String expectedKey, UUID traceId) throws Exception {
        idGenerator.setNextIds(traceId);
        var result = mockMvc
