...
class LedgerReconciliationConcurrencyTest {
    ...
    void concurrentAdjustmentCommitsSerializeAndOnlyOneVersionedWinnerPostsMoney() throws Exception {
        ...
        try {
            var first = executor.submit(() -> runCommit(
                    start, ownerId, account.id(), preview.projectionVersion(), UUID.randomUUID(), "concurrent one"));
            var second = executor.submit(() -> runCommit(
                    start, ownerId, account.id(), preview.projectionVersion(), UUID.randomUUID(), "concurrent two"));
            start.countDown();
            ...
            assertThat(outcomes.stream().map(Outcome::errorCode).filter(java.util.Objects::nonNull))
                    .containsExactly(LedgerErrorCode.BALANCE_VERSION_CONFLICT);
    ...
    void duplicateConcurrentRetriesReplayOneEconomicResult() throws Exception {
        ...
        try {
            var first = executor.submit(() ->
                    runCommit(start, ownerId, account.id(), preview.projectionVersion(), requestId, "same request"));
            ...
            assertThat(outcomes)
                    ...
                    .containsExactly(
                            outcomes.getFirst().id(), outcomes.getFirst().id());
            assertThat(jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM ledger.reconciliation WHERE owner_user_account_id = ?",
                            Integer.class,
                            ownerId))
    ...
    void duplicateConcurrentCorrectionRetriesReplayOneReplacement() throws Exception {
        ...
        try {
            var first = executor.submit(() ->
                    runCorrection(start, ownerId, original.id(), replacementPreview.projectionVersion(), requestId));
            var second = executor.submit(() ->
                    runCorrection(start, ownerId, original.id(), replacementPreview.projectionVersion(), requestId));
            start.countDown();
            var outcomes = List.of(first.get(15, TimeUnit.SECONDS), second.get(15, TimeUnit.SECONDS));
            ...
            assertThat(outcomes)
                    .extracting(Outcome::id)
                    .containsOnly(outcomes.getFirst().id());
            assertThat(jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM ledger.reconciliation WHERE owner_user_account_id = ?",
                            Integer.class,
                            ownerId))
    ...
    void concurrentCorrectionsAllowOnlyOneDirectSuperseder() throws Exception {
        ...
        try {
            var first = executor.submit(() -> runCorrection(
                    start, ownerId, original.id(), replacementPreview.projectionVersion(), UUID.randomUUID()));
    ...
    private Outcome runCommit(
            CountDownLatch start, UUID ownerId, UUID accountId, long version, UUID requestId, String reason) {
        await(start);
        try {
            var response = new TransactionTemplate(transactionManager)
                    .execute(status -> reconciliationService.commit(
                            ownerId,
                            accountId,
                            new ReconciliationCommitRequest(
                                    "concurrent",
    ...
    private Outcome runCorrection(
            CountDownLatch start, UUID ownerId, UUID reconciliationId, long version, UUID requestId) {
        await(start);
        try {
            var response = new TransactionTemplate(transactionManager)
                    .execute(status -> reconciliationService.correct(
                            ownerId,
                            reconciliationId,
    ...
    private FinancialAccountResponse createAccount(UUID ownerId, String name, String amount, Instant openingAt) {
        return accountService.create(
                ...
                new CreateFinancialAccountRequest(
                        ...
                        null,
                        new dev.canverse.stocks.ledger.web.request.OpeningStateRequest(amount, openingAt)));
    }
